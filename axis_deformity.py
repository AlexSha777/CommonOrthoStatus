...
class AxisWindow(QWidget):
    ...
    def but_detail_process(self):
        ...
        if self.combo_box_new == None:
            self.deform_directions = ["вальгус","варус","антекурвация","рекурвация", "вальгус-антекурвация","варус-антекурвация","вальгус-рекурвация","варус-рекурвация"]
            self.deform_directions_but = []
            self.deform_degree = ["5","10","20","30","40","50", "60"]
            ...
            self.combo_box_new.resize(self.combo_width*1.25, self.sender().size().height()*10)

            if but_name[:12]=="but_det_left": 
    ...
    def combo_box_def_direction(self, index=None):
        ...
        text_axis = ""
        text_degree = ""
        if text in ["вальгус","варус","антекурвация","рекурвация", "вальгус-антекурвация","варус-антекурвация","вальгус-рекурвация","варус-рекурвация"]:
            self.button_axis.append(self.sender())
            text_axis = text
            for item in self.deform_directions_but: item.color = "#FF7373"
            self.sender().color = "#BF3030"

        elif text in ["5","10","20","30","40","50", "60"]:
            self.button_degree.append(self.sender())
            text_degree = text
    ...
    def text_formation(self):
        ...
        ["вальгус","варус","антекурвация","рекурвация", "вальгус-антекурвация","варус-антекурвация","вальгус-рекурвация","варус-рекурвация"]


        if right_but_hand != [] or left_but_hand != [] or right_but_leg != [] or left_but_leg != []:
            text += "Определяется деформация оси: "
            inc = 0
            for part in [right_but_hand,right_but_leg, left_but_hand, left_but_leg]:
                if part ==[]:
                    ...
                    for but in part:
                        text += " в проекции " + self.localize[but[0].objectName()[-2:]] + " " + but[1]["axis"] + " в пределах " + but[1]["degree"] + self.degree_sign +", "
        ...
        print(right_but_leg)
        print(left_but_leg)
        self.label_main_text.setText(text[:-2]+". ")
        self.update()
